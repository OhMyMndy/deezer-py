...
class GW:
    ...
    def api_call(self, method, args=None, params=None):
        ...
        if params is None:
            params = {}
        if not self.api_token and method != "deezer.getUserData":
        ...
        if len(result_json["error"]):
            ...
            } or result_json["error"] == {"VALID_TOKEN_REQUIRED": "Invalid CSRF token"}:
                self.api_token = self._get_token()
                return self.api_call(method, args, params)
            if result_json.get("payload", {}) and result_json["payload"].get(
                "FALLBACK", {}
            ):
        ...
        if not self.api_token and method == "deezer.getUserData":
            self.api_token = result_json["results"]["checkForm"]
    ...
    def get_user_profile_page(self, user_id, tab, limit=10):
        return self.api_call(
            "deezer.pageProfile", {"USER_ID": user_id, "tab": tab, "nb": limit}
        )

    def get_user_favorite_ids(self, checksum=None, limit=10000, start=0):
        return self.api_call(
            "song.getFavoriteIds", {"nb": limit, "start": start, "checksum": checksum}
        )
    ...
    def get_artist_page(self, art_id):
        return self.api_call(
            "deezer.pageArtist",
            {"ART_ID": art_id, "lang": "en", "header": True, "tab": 0},
        )
    ...
    def get_playlist_page(self, playlist_id):
        return self.api_call(
            "deezer.pagePlaylist",
            {"PLAYLIST_ID": playlist_id, "lang": "en", "header": True, "tab": 0},
        )

    def get_playlist_tracks(self, playlist_id):
        tracks_array = []
        body = self.api_call(
            "playlist.getSongs", {"PLAYLIST_ID": playlist_id, "nb": -1}
        )
        for track in body["data"]:
            track["POSITION"] = body["data"].index(track)
            tracks_array.append(track)
    ...
    def create_playlist(
        self, title, status=PlaylistStatus.PUBLIC, description=None, songs=[]
    ):
        newSongs = []
        for song in songs:
    ...
    def edit_playlist(
        self, playlist_id, title, status=None, description=None, songs=[]
    ):
        newSongs = []
        for song in songs:
            newSongs.append([song, 0])
    ...
    def add_songs_to_playlist(self, playlist_id, songs, offset=-1):
        ...
        return self.api_call(
            "playlist.addSongs",
            {"PLAYLIST_ID": playlist_id, "songs": newSongs, "offset": offset},
        )
    ...
    def remove_songs_from_playlist(self, playlist_id, songs):
        ...
        for song in songs:
            newSongs.append([song, 0])
        return self.api_call(
            "playlist.deleteSongs", {"PLAYLIST_ID": playlist_id, "songs": newSongs}
        )
    ...
    def add_playlist_to_favorites(self, playlist_id):
        return self.api_call(
            "playlist.addFavorite", {"PARENT_PLAYLIST_ID": playlist_id}
        )
        ...
        for release in releases:
            if release["ALB_ID"] not in ids:
                ...
                if (
                    ...
                    or release["ART_ID"] != art_id
                    and release["ROLE_ID"] == 0
                ) and release["ARTISTS_ALBUMS_IS_OFFICIAL"]:
    ...
    def get_user_playlists(self, user_id, limit=25):
        user_profile_page = self.get_user_profile_page(
            user_id, "playlists", limit=limit
        )
    ...
    def get_user_albums(self, user_id, limit=25):
        data = self.get_user_profile_page(user_id, "albums", limit=limit)["TAB"][
            "albums"
        ]["data"]
        result = []
        for album in data:
    ...
    def get_user_artists(self, user_id, limit=25):
        data = self.get_user_profile_page(user_id, "artists", limit=limit)["TAB"][
            "artists"
        ]["data"]
        result = []
        for artist in data:
    ...
    def get_user_tracks(self, user_id, limit=25):
        ...
        if user_data["USER"]["USER_ID"] == user_id:
            return self.get_my_favorite_tracks()
        data = self.get_user_profile_page(user_id, "loved", limit=limit)["TAB"][
            "loved"
        ]["data"]
        result = []
        for track in data:
