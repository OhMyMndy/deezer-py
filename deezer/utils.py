...
def is_explicit(explicit_content_lyrics):
    ...
    return explicit_content_lyrics or LyricsStatus.UNKNOWN in [
        LyricsStatus.EXPLICIT,
        LyricsStatus.PARTIALLY_EXPLICIT,
    ]
...
def map_user_track(track):
    ...
    result = {
        ...
        "album": {
            ...
            "tracklist": "https://api.deezer.com/album/"
            + str(track["ALB_ID"])
            + "/tracks",
            "type": "album",
        },
        "artist": {
            "id": track["ART_ID"],
            "name": track["ART_NAME"],
            "picture": "https://api.deezer.com/artist/"
            + str(track["ART_ID"])
            + "/image",
            "picture_small": None,
            "picture_medium": None,
            "picture_big": None,
            "picture_xl": None,
            "tracklist": "https://api.deezer.com/artist/"
            + str(track["ART_ID"])
            + "/top?limit=50",
            "type": "artist",
        },
    ...
    if int(track["SNG_ID"]) >= 0:
        ...
        result["explicit_content_lyrics"] = track.get("EXPLICIT_TRACK_CONTENT", {}).get(
            "EXPLICIT_COVER_STATUS"
        )
        result["explicit_content_cover"] = track.get("EXPLICIT_TRACK_CONTENT", {}).get(
...
def map_user_artist(artist):
    ...
    return {
        ...
        + str(artist["ART_PICTURE"])
        + "/56x56-000000-80-0-0.jpg",
        "picture_medium": "https://e-cdns-images.dzcdn.net/images/artist/"
        + str(artist["ART_PICTURE"])
        + "/250x250-000000-80-0-0.jpg",
        "picture_big": "https://e-cdns-images.dzcdn.net/images/artist/"
        + str(artist["ART_PICTURE"])
        + "/500x500-000000-80-0-0.jpg",
        "picture_xl": "https://e-cdns-images.dzcdn.net/images/artist/"
        + str(artist["ART_PICTURE"])
        + "/1000x1000-000000-80-0-0.jpg",
...
def map_user_playlist(playlist, default_user_name=""):
    ...
    return {
        ...
        "picture": "https://api.deezer.com/playlist/"
        + str(playlist["PLAYLIST_ID"])
        + "/image",
        "picture_small": "https://e-cdns-images.dzcdn.net/images/"
        + playlist["PICTURE_TYPE"]
        + "/"
        + playlist["PLAYLIST_PICTURE"]
        + "/56x56-000000-80-0-0.jpg",
        "picture_medium": "https://e-cdns-images.dzcdn.net/images/"
        + playlist["PICTURE_TYPE"]
        + "/"
        + playlist["PLAYLIST_PICTURE"]
        + "/250x250-000000-80-0-0.jpg",
        "picture_big": "https://e-cdns-images.dzcdn.net/images/"
        + playlist["PICTURE_TYPE"]
        + "/"
        + playlist["PLAYLIST_PICTURE"]
        + "/500x500-000000-80-0-0.jpg",
        "picture_xl": "https://e-cdns-images.dzcdn.net/images/"
        + playlist["PICTURE_TYPE"]
        + "/"
        + playlist["PLAYLIST_PICTURE"]
        + "/1000x1000-000000-80-0-0.jpg",
        "tracklist": "https://api.deezer.com/playlist/"
        + str(playlist["PLAYLIST_ID"])
        + "/tracks",
...
def map_album(album):
    ...
    result = {
        ...
        "explicit_content_lyrics": album.get("EXPLICIT_ALBUM_CONTENT", {}).get(
            "EXPLICIT_LYRICS_STATUS"
        ),
        "explicit_content_cover": album.get("EXPLICIT_ALBUM_CONTENT", {}).get(
            "EXPLICIT_COVER_STATUS"
        ),
        "contributors": [],
        "artist": {
    ...
    if (
        ...
        and result["title_version"] in result["title_short"]
    ):
        result["title_short"] = (
            result["title_short"].replace(result["title_version"], "").strip()
        )
    ...
    if album.get("NUMBER_DISK"):
        result["nb_disk"] = album["NUMBER_DISK"]
    result["copyright"] = album.get("COPYRIGHT")
    if "ARTISTS" in album:
        for contributor in album["ARTISTS"]:
            if contributor["ART_ID"] == result["artist"]["id"]:
                result["artist"]["picture_small"] = (
                    f"https://e-cdns-images.dzcdn.net/images/artist/{contributor['ART_PICTURE']}/56x56-000000-80-0-0.jpg"
                )
                result["artist"]["picture_medium"] = (
                    f"https://e-cdns-images.dzcdn.net/images/artist/{contributor['ART_PICTURE']}/250x250-000000-80-0-0.jpg"
                )
                result["artist"]["picture_big"] = (
                    f"https://e-cdns-images.dzcdn.net/images/artist/{contributor['ART_PICTURE']}/500x500-000000-80-0-0.jpg"
                )
                result["artist"]["picture_xl"] = (
                    f"https://e-cdns-images.dzcdn.net/images/artist/{contributor['ART_PICTURE']}/1000x1000-000000-80-0-0.jpg"
                )
...
def map_artist_album(album):
    ...
    return {
        ...
        "explicit_content_cover": album["EXPLICIT_ALBUM_CONTENT"][
            "EXPLICIT_LYRICS_STATUS"
        ],
        "explicit_content_lyrics": album["EXPLICIT_ALBUM_CONTENT"][
            "EXPLICIT_COVER_STATUS"
        ],
...
def map_playlist(playlist):
    ...
    return {
        ...
        + "/image",
        "picture_small": "https://cdns-images.dzcdn.net/images/"
        + playlist["PICTURE_TYPE"]
        + "/"
        + playlist["PLAYLIST_PICTURE"]
        + "/56x56-000000-80-0-0.jpg",
        "picture_medium": "https://cdns-images.dzcdn.net/images/"
        + playlist["PICTURE_TYPE"]
        + "/"
        + playlist["PLAYLIST_PICTURE"]
        + "/250x250-000000-80-0-0.jpg",
        "picture_big": "https://cdns-images.dzcdn.net/images/"
        + playlist["PICTURE_TYPE"]
        + "/"
        + playlist["PLAYLIST_PICTURE"]
        + "/500x500-000000-80-0-0.jpg",
        "picture_xl": "https://cdns-images.dzcdn.net/images/"
        + playlist["PICTURE_TYPE"]
        + "/"
        + playlist["PLAYLIST_PICTURE"]
        + "/1000x1000-000000-80-0-0.jpg",
        ...
        "creator": {
            ...
            "tracklist": "https://api.deezer.com/user/"
            + playlist["PARENT_USER_ID"]
            + "/flow",
            "type": "user",
        },
        "type": "playlist",
...
def map_track(track):
    ...
    result = {
        ...
        "filesizes": {"default": track["FILESIZE"]},
    ...
    if int(track["SNG_ID"]) > 0:
        ...
        if "FALLBACK" in track:
            result["fallback_id"] = track.get("FALLBACK").get("SNG_ID")
        if "DIGITAL_RELEASE_DATE" in track:
            result["digital_release_date"] = track["DIGITAL_RELEASE_DATE"]
        if "GENRE_ID" in track:
            result["genre_id"] = track["GENRE_ID"]
        if "COPYRIGHT" in track:
            result["copyright"] = track["COPYRIGHT"]
        if "LYRICS" in track:
            result["lyrics"] = track["LYRICS"]
        if "ALBUM_FALLBACK" in track:
            result["alternative_albums"] = track["ALBUM_FALLBACK"]
