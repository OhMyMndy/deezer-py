...
class Deezer:
    ...
    def _post_login(self, user_data):
        ...
        family = (
            ...
            and not user_data["USER"]["MULTI_ACCOUNT"]["IS_SUB_ACCOUNT"]
        )
        if family:
    ...
    def change_account(self, child_n):
        ...
        if lang[2:1] == "-":
            ...
            lang = lang[0:2]
        self.http_headers["Accept-Language"] = re.sub(
            r"[^0-9A-Za-z *,-.;=]", "", str(self.current_user["language"])
        )

        return (self.current_user, self.selected_account)

    def get_track_url(self, track_token, track_format):
        tracks = self.get_tracks_url(
            [
                track_token,
            ],
            track_format,
    ...
    def get_tracks_url(self, track_tokens, track_format):
        ...
        if not self.current_user.get("license_token"):
            return []
        if (
            (track_format == "FLAC" or track_format.startswith("MP4_RA"))
            and not self.current_user.get("can_stream_lossless")
            or track_format == "MP3_320"
            and not self.current_user.get("can_stream_hq")
        ):
