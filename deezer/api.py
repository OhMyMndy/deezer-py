...
class API:
        ...
        if "error" in result_json.keys():
            if "code" in result_json["error"]:
                ...
                if result_json["error"]["code"] == 100:
                    raise ItemsLimitExceededException(
                ...
                if result_json["error"]["code"] == 901:
                    raise IndividualAccountChangedNotAllowedException(
    ...
    def get_album_comments(self, album_id, index=0, limit=10):
        return self.api_call(
            f"album/{str(album_id)}/comments", {"index": index, "limit": limit}
        )

    def get_album_fans(self, album_id, index=0, limit=100):
        return self.api_call(
            f"album/{str(album_id)}/fans", {"index": index, "limit": limit}
        )

    def get_album_tracks(self, album_id, index=0, limit=-1):
        return self.api_call(
            f"album/{str(album_id)}/tracks", {"index": index, "limit": limit}
        )
    ...
    def get_artist_top(self, artist_id, index=0, limit=10):
        return self.api_call(
            f"artist/{str(artist_id)}/top", {"index": index, "limit": limit}
        )

    def get_artist_albums(self, artist_id, index=0, limit=-1):
        return self.api_call(
            f"artist/{str(artist_id)}/albums", {"index": index, "limit": limit}
        )

    def get_artist_comments(self, artist_id, index=0, limit=10):
        return self.api_call(
            f"artist/{str(artist_id)}/comments", {"index": index, "limit": limit}
        )

    def get_artist_fans(self, artist_id, index=0, limit=100):
        return self.api_call(
            f"artist/{str(artist_id)}/fans", {"index": index, "limit": limit}
        )

    def get_artist_related(self, artist_id, index=0, limit=20):
        return self.api_call(
            f"artist/{str(artist_id)}/related", {"index": index, "limit": limit}
        )

    def get_artist_radio(self, artist_id, index=0, limit=25):
        return self.api_call(
            f"artist/{str(artist_id)}/radio", {"index": index, "limit": limit}
        )

    def get_artist_playlists(self, artist_id, index=0, limit=-1):
        return self.api_call(
            f"artist/{str(artist_id)}/playlists", {"index": index, "limit": limit}
        )
    ...
    def get_chart_tracks(self, genre_id=0, index=0, limit=10):
        return self.api_call(
            f"chart/{str(genre_id)}/tracks", {"index": index, "limit": limit}
        )

    def get_chart_albums(self, genre_id=0, index=0, limit=10):
        return self.api_call(
            f"chart/{str(genre_id)}/albums", {"index": index, "limit": limit}
        )

    def get_chart_artists(self, genre_id=0, index=0, limit=10):
        return self.api_call(
            f"chart/{str(genre_id)}/artists", {"index": index, "limit": limit}
        )

    def get_chart_playlists(self, genre_id=0, index=0, limit=10):
        return self.api_call(
            f"chart/{str(genre_id)}/playlists", {"index": index, "limit": limit}
        )

    def get_chart_podcasts(self, genre_id=0, index=0, limit=10):
        return self.api_call(
            f"chart/{str(genre_id)}/podcasts", {"index": index, "limit": limit}
        )
    ...
    def get_editorial_selection(self, genre_id=0, index=0, limit=10):
        return self.api_call(
            f"editorial/{str(genre_id)}/selection", {"index": index, "limit": limit}
        )

    def get_editorial_charts(self, genre_id=0, index=0, limit=10):
        return self.api_call(
            f"editorial/{str(genre_id)}/charts", {"index": index, "limit": limit}
        )

    def get_editorial_releases(self, genre_id=0, index=0, limit=10):
        return self.api_call(
            f"editorial/{str(genre_id)}/releases", {"index": index, "limit": limit}
        )
    ...
    def get_genre_artists(self, genre_id=0, index=0, limit=10):
        return self.api_call(
            f"genre/{str(genre_id)}/artists", {"index": index, "limit": limit}
        )

    def get_genre_radios(self, genre_id=0, index=0, limit=10):
        return self.api_call(
            f"genre/{str(genre_id)}/radios", {"index": index, "limit": limit}
        )
    ...
    def get_playlist_comments(self, album_id, index=0, limit=10):
        return self.api_call(
            f"playlist/{str(album_id)}/comments", {"index": index, "limit": limit}
        )

    def get_playlist_fans(self, album_id, index=0, limit=100):
        return self.api_call(
            f"playlist/{str(album_id)}/fans", {"index": index, "limit": limit}
        )

    def get_playlist_tracks(self, album_id, index=0, limit=-1):
        return self.api_call(
            f"playlist/{str(album_id)}/tracks", {"index": index, "limit": limit}
        )

    def get_playlist_radio(self, album_id, index=0, limit=100):
        return self.api_call(
            f"playlist/{str(album_id)}/radio", {"index": index, "limit": limit}
        )
    ...
    def get_radio_tracks(self, radio_id, index=0, limit=40):
        return self.api_call(
            f"radio/{str(radio_id)}/tracks", {"index": index, "limit": limit}
        )

    def _generate_search_advanced_query(
        self,
        artist="",
        album="",
        track="",
        ...
        if label:
            query += f'label:"{label}" '
        if dur_min:
            query += f'dur_min:"{str(dur_min)}" '
        if dur_max:
            query += f'dur_max:"{str(dur_max)}" '
        if bpm_min:
            query += f'bpm_min:"{str(bpm_min)}" '
        if bpm_max:
            query += f'bpm_max:"{str(bpm_max)}" '
        return query.strip()
    ...
    def advanced_search(
        ...
        query = self._generate_search_advanced_query(
            artist, album, track, label, dur_min, dur_max, bpm_min, bpm_max
        )
    ...
    def get_user_albums(self, user_id, index=0, limit=25):
        return self.api_call(
            f"user/{str(user_id)}/albums", {"index": index, "limit": limit}
        )

    def get_user_artists(self, user_id, index=0, limit=25):
        return self.api_call(
            f"user/{str(user_id)}/artists", {"index": index, "limit": limit}
        )

    def get_user_flow(self, user_id, index=0, limit=25):
        return self.api_call(
            f"user/{str(user_id)}/flow", {"index": index, "limit": limit}
        )

    def get_user_following(self, user_id, index=0, limit=25):
        return self.api_call(
            f"user/{str(user_id)}/followings", {"index": index, "limit": limit}
        )

    def get_user_followers(self, user_id, index=0, limit=25):
        return self.api_call(
            f"user/{str(user_id)}/followers", {"index": index, "limit": limit}
        )

    def get_user_playlists(self, user_id, index=0, limit=25):
        return self.api_call(
            f"user/{str(user_id)}/playlists", {"index": index, "limit": limit}
        )

    def get_user_radios(self, user_id, index=0, limit=25):
        return self.api_call(
            f"user/{str(user_id)}/radios", {"index": index, "limit": limit}
        )

    def get_user_tracks(self, user_id, index=0, limit=25):
        return self.api_call(
            f"user/{str(user_id)}/tracks", {"index": index, "limit": limit}
        )
    ...
    def get_countries_charts(self):
        ...
        if not result[0]["title"].startswith("Top"):
            result = result[1:]  # Remove loved tracks playlist
        return result
    ...
    def get_track_id_from_metadata(self, artist, track, album):
        ...
        if "(" in track and ")" in track and track.find("(") < track.find(")"):
            resp = self.advanced_search(
                artist=artist, track=track[: track.find("(")], limit=1
            )
            if len(resp["data"]) > 0:
                return resp["data"][0]["id"]
        elif " - " in track:
